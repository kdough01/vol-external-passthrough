                             ...
                             struct pressio **out_lib)
{
    ...
    if (ctx->stream) {
        ...
        (void)pressio_compressor_set_options(chunk, sopt);
...
{
    herr_t ret_val = 0;
    struct pressio *lib = NULL;
    struct pressio_compressor *chunk = NULL;
    ...
    *out_csize = 0;
    ...
    if (!chunk) {
        H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__,
                vol_err_class, maj_compression, min_compress_failed,
                "failed to build 'chunking' meta-compressor for '%s' "
                "(is chunking compiled into libpressio?)", ctx->compressor_id);
        return -1;
    }

#ifdef ENABLE_EXT_PASSTHRU_LOGGING
    printf("------- COMPRESS NATIVE: id=%s nbytes=%zu nelem=%zu chunk_elems=%zu\n",
           ctx->compressor_id, nbytes, in_dims[0],
           vol_native_chunk_elems(ctx, dsize));
#endif

    /* Host memory in; GPU compressors migrate via the domain manager. */
    input = pressio_data_new_nonowning_domain(in_dtype, (void *)data,
                                              1, in_dims, vol_ptr_domain(data));
...
#ifdef USE_CUDA
    {
        ...
        int _cerr = pressio_compressor_compress(chunk, input, output);
        ...
        if (_cerr) {
            H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__,
                    vol_err_class, maj_compression, min_compress_failed,
                    "chunking compress failed for '%s' (%zu B): %s",
                    ctx->compressor_id, nbytes,
                    pressio_compressor_error_msg(chunk));
            ret_val = -1;
            goto done;
        }
    }
#else
    if (pressio_compressor_compress(chunk, input, output)) {
        H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__,
                vol_err_class, maj_compression, min_compress_failed,
                "chunking compress failed for '%s' (%zu B): %s",
                ctx->compressor_id, nbytes,
...

    {
        ...
        if (comp_size == 0 || comp_ptr == NULL) {
            H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__,
                    vol_err_class, maj_compression, min_compress_failed,
                    "chunking compress of '%s' produced 0 bytes for %zu B",
                    ctx->compressor_id, nbytes);
            ret_val = -1;
            goto done;
        }
    ...
    if (getenv("HDF5_VOL_PRESSIO_METRICS")) {
        struct pressio_options *results =
            pressio_compressor_get_metrics_results(chunk);
        char *str = pressio_options_to_string(results);
    ...
    if (input)  pressio_data_free(input);
    if (output) pressio_data_free(output);
    if (chunk)  pressio_compressor_release(chunk);
                                        ...
                                        void *out, size_t out_bytes)
{
...
#ifdef ENABLE_EXT_PASSTHRU_LOGGING
    printf("------- DECOMPRESS NATIVE: id=%s csize=%zu out_bytes=%zu nelem=%zu\n",
           ctx->compressor_id, csize, out_bytes, out_dims[0]);
#endif
...
#ifdef USE_CUDA
    {
        ...
        int _derr = pressio_compressor_decompress(chunk, input, output);
        ...
        if (_derr) {
            H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__,
                    vol_err_class, maj_compression, min_decompress_failed,
                    "chunking decompress failed for '%s' (%zu B): %s",
                    ctx->compressor_id, csize,
                    pressio_compressor_error_msg(chunk));
            ret_val = -1;
            goto done;
...
#else
    if (pressio_compressor_decompress(chunk, input, output)) {
        H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__,
                vol_err_class, maj_compression, min_decompress_failed,
                "chunking decompress failed for '%s' (%zu B): %s",
                ctx->compressor_id, csize,
...

    {
        ...
        if (!out_ptr || actual_bytes == 0) {
            H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__,
                    vol_err_class, maj_compression, min_decompress_failed,
                    "chunking decompress of '%s' produced no host output",
                    ctx->compressor_id);
            ret_val = -1;
            goto done;
        }
        if (actual_bytes != out_bytes) {
            H5Epush(H5E_DEFAULT, __FILE__, __func__, __LINE__,
                    vol_err_class, maj_compression, min_decompress_failed,
                    "chunking decompress of '%s' produced %zu bytes, expected %zu",
                    ctx->compressor_id, actual_bytes, out_bytes);
            ret_val = -1;
            goto done;
        }
